...
This script calculates the Bond-Network Entropy (BNE) for a given structure and saves the result to HDF5 files.  

How to run
----------
From the repository root:

    python workflows/5_BNE_workflow.py
...
from config import (
    BNE_WORK_DIR, BNE_FOLDER,
    DISORDERED_POSCAR,
    CUTOFF, N_SMALLEST, STRUCTURE_IDX, LOCAL_ENVIRONMENT_NAT,
)
...
if __name__ == "__main__":
    ...
    save_folder = f"{BNE_WORK_DIR}/{BNE_FOLDER}"

    # Input structure: silica glass with 5184 atoms.
    # Downloaded from https://www.pnas.org/doi/abs/10.1073/pnas.2422763122
    atomic_positions, lattice_vectors = obtain_positions_and_lattice_vectors(DISORDERED_POSCAR)
    n_atoms = len(atomic_positions)

    # Compute nearest-neighbour distances for every atom.
    distances, idx_distances = obtain_distances_big_structures(atomic_positions, lattice_vectors, n_smallest=N_SMALLEST)

    # Build the adjacency matrix:
    adjacency_matrix = ((distances < CUTOFF) & (distances > 0.1)).astype(int)

    calculate_barcode(distances, idx_distances, adjacency_matrix, n_atoms, save_folder, STRUCTURE_IDX, DISORDERED_POSCAR, LOCAL_ENVIRONMENT_NAT)